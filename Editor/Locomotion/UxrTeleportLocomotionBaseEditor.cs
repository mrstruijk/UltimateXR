...
namespace UltimateXR.Editor.Locomotion
{
    ...
    public abstract class UxrTeleportLocomotionBaseEditor : UnityEditor.Editor
    {
        ...
        private GUIContent ContentControllerHand { get; } = new("Controller Hand", "Which hand controls the input");
        private GUIContent ContentUseControllerForward { get; } = new("Use Controller Forward", "Will the teleport use the controller's forward vector instead of its own transform forward?");
        private GUIContent ContentParentToDestination { get; } = new("Parent To Destination", "Whether to parent the avatar to the destination object after teleport. Use it when building applications with moving vehicles or platforms the avatar can move on, so that the avatar keeps the relative position/orientation after teleporting.");
        private GUIContent ContentShakeFilter { get; } = new("Shake Filter", "The amount of filtering to apply to the hand movement to smooth it out");
        private GUIContent ContentTranslationType { get; } = new("Translation Type", "Which translation method to use");
        private GUIContent ContentFadeTranslationColor { get; } = new("Translation Fade Color", "The fade color when Fade translation type is used");
        private GUIContent ContentFadeTranslationSeconds { get; } = new("Translation Fade Seconds", "The fade transition in seconds when Fade translation type is used");
        private GUIContent ContentSmoothTranslationSeconds { get; } = new("Smooth Translation Seconds", "The translation duration in seconds when Smooth translation type is used");
        private GUIContent ContentAllowJoystickBackStep { get; } = new("Allow Joystick Back Step", "Whether to allow back steps by pressing the joystick down");
        private GUIContent ContentBackStepDistance { get; } = new("Back Step Distance", "The distance of each back step");
        private GUIContent ContentRotationType { get; } = new("Rotation Type", "Which rotation method to use");
        private GUIContent ContentRotationStepDegrees { get; } = new("Rotation Step Degrees", "The amount of degrees of each turn for Immediate and Fade rotations");
        private GUIContent ContentFadeRotationColor { get; } = new("Rotation Fade Color", "The fade color when Fade rotation is used");
        private GUIContent ContentFadeRotationSeconds { get; } = new("Rotation Fade Seconds", "The fade transition in seconds when Fade rotation is used");
        private GUIContent ContentSmoothRotationSeconds { get; } = new("Smooth Rotation Seconds", "The rotation duration in seconds when Smooth rotation is used");
        private GUIContent ContentReorientationType { get; } = new("Reorient After Teleport", "How to orient the view right after teleporting");
        private GUIContent ContentTarget { get; } = new("Target", "Which teleport target to use. Can be either a prefab or an already instantiated object");
        private GUIContent ContentTargetPlacementAboveHit { get; } = new("Target Placement Above Floor", "Offset applied to the teleport target to help placing it a little above the floor");
        private GUIContent ContentShowTargetAlsoWhenInvalid { get; } = new("Show Target Also When Invalid", "Whether to show the target object also when the destination is not valid");
        private GUIContent ContentValidMaterialColorTargets { get; } = new("Target Color When Valid", "Target color to use when the destination is valid");
        private GUIContent ContentInvalidMaterialColorTargets { get; } = new("Target Color When Invalid", "Target color to use when the destination is not valid");
        private GUIContent ContentTriggerCollidersInteraction { get; } = new("Trigger Colliders Interaction", "Whether colliders with the trigger property set will interact with the teleport raycasts");
        private GUIContent ContentMaxAllowedDistance { get; } = new("Max Allowed Distance Travel", "Maximum allowed distance that can be travelled using each teleport");
        private GUIContent ContentMaxAllowedHeightDifference { get; } = new("Max Allowed Height Difference", "Maximum allowed height difference to be able to teleport");
        private GUIContent ContentMaxAllowedSlopeDegrees { get; } = new("Max Allowed Slope Degrees", "Maximum allowed slope degrees at destination");
        private GUIContent ContentDestinationValidationRadius { get; } = new("Destination Validation Radius", "Radius of a cylinder that will be used to validate the destination surroundings to allow teleporting");
        private GUIContent ContentValidTargetLayers { get; } = new("Valid Target Layers", "Valid layers for teleporting destination objects");
        private GUIContent ContentBlockingTargetLayers { get; } = new("Blocking Target Layers", "Objects that will block teleporting raycasts");
