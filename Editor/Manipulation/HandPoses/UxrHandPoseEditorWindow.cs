...
namespace UltimateXR.Editor.Manipulation.HandPoses
{
    ...
    public partial class UxrHandPoseEditorWindow : EditorWindow
    {
        ...
        {
            var handPoseWindow = GetWindow(typeof(UxrHandPoseEditorWindow), true, "UltimateXR Hand Pose Editor");
        ...
        public static UxrHandPoseType GetPoseType(string poseName, UxrAvatar avatar)
        {
            ...
            var handPose = avatar.GetHandPose(poseName);
        ...
        public static void Open(UxrAvatar avatar, UxrHandPoseAsset handPose = null)
        {
            ...
            var handPoseWindow = GetWindow(typeof(UxrHandPoseEditorWindow)) as UxrHandPoseEditorWindow;
        ...
        {
            var lastY = 0;
            var registerUndoOnPoseUpdate = true;
            ...
            if (_foldoutMainMenu)
            {
                ...
                var newAvatar = EditorGUILayout.ObjectField(new GUIContent("Target Avatar", "Drop an avatar from the scene here to edit its hand poses"), _avatar, typeof(UxrAvatar), true, GUILayout.Width(FieldWidth)) as UxrAvatar;
                ...
                if (_avatar != null && _avatar.IsPrefabVariant)
                {
                    ...
                    foreach (var avatarPrefab in _avatar.GetPrefabChain().Reverse())
                    {
                        var posY = lastY + (prefabIndex + 1) * VerticalHeightSingleLine;
                ...
                if (GUI.Button(GetPosesMenuButtonRect(PosesMenuColumnCreate, lastY), new GUIContent("Create New Pose...", "Creates a new pose for the currently selected avatar animator")))
                {
                    ...
                    if (CheckWarnSaveHandPose(_currentHandPose, SaveHandPoseFlags.HandDescriptors))
                    {
                        var newHandPoseAsset = CreatePose(_avatar);
                ...
                if (GUI.Button(GetPosesMenuButtonRect(PosesMenuColumnCreate, lastY), new GUIContent("Copy Current Pose To New...", "Copies the current pose to a new pose asset file")))
                {
                    ...
                    if (CheckWarnSaveHandPose(_currentHandPose, SaveHandPoseFlags.HandDescriptors))
                    {
                        var newHandPoseAsset = CopyPoseAsNew(_avatar, _currentHandPose);
                ...
                {
                    var message = _avatar.IsPrefabVariant ? $"Delete all poses in {_avatar.GetAvatarPrefab().name}? Inherited poses from parent prefabs are read-only and will not be deleted." : "Delete all poses?";

                    if (EditorUtility.DisplayDialog("Confirm", message, "Yes", "Cancel"))
                    {
                        foreach (var poseName in _avatar.GetHandPoses().Select(p => p.name))
                        {
                ...
                {
                    var path = EditorUtility.OpenFilePanel("Open existing pose", CurrentLoadFolder, "asset");
                    ...
                    if (!string.IsNullOrEmpty(path))
                    {
                        ...
                        var srcHandPoseAsset = AssetDatabase.LoadAssetAtPath<UxrHandPoseAsset>(path);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (save)
                                {
                                    var newHandPoseAsset = CreatePose(_avatar, false, path);
                ...
                {
                    var pathSrc = EditorUtility.OpenFolderPanel("Pose Assets Source Folder", CurrentLoadFolder, string.Empty);
                    ...
                    {
                        var handPosesToAdd = new List<UxrHandPoseAsset>();
                        var files = Directory.GetFiles(pathSrc);
                        ...
                        foreach (var file in files)
                        {
                            ...
                            var fixedFile = UxrEditorUtils.ToHandPoseAssetPath(file);
                        ...
                        {
                            var pathDst = EditorUtility.SaveFolderPanel("Pose Assets Destination Folder", CurrentSaveFolder, string.Empty);
                            ...
                            {
                                foreach (var srcHandPoseAsset in handPosesToAdd)
                                {
                                    var newHandPoseAsset = CreatePose(_avatar, false, pathDst + "/" + srcHandPoseAsset.name + ".asset");
            ...
            if (_foldoutPoseMenu)
            {
                    ...
                    if (_avatar.IsPrefabVariant)
                    {
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            ...
                            if (_showInheritedPoses == false && _currentHandPose != null)
                            {
                                ...
                                var handPose = _avatar.GetHandPose(_currentHandPose.name, false);
                ...
                if (_avatar != null && _currentHandPose != null && _currentPoseNames.Count > 0)
                {
                    ...
                    if (_avatar.IsHandPoseOverriden(_currentHandPose.name, out var originalAvatar))
                    {
                ...
                EditorGUI.BeginChangeCheck();
                var oldPoseTypeIndex = _poseTypeIndex;
                ...
                if (CurrentPoseType == UxrHandPoseType.Blend)
                {
                    ...
                    var blendPoseIndex = EditorGUILayout.Popup(new GUIContent("Edit Blend Pose", "The blend pose currently being edited"),
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        if (CheckWarnSaveHandPose(_currentHandPose, SaveHandPoseFlags.HandDescriptors))
                        {
                            var blendPoseType = blendPoseIndex != -1 ? (UxrBlendPoseType) Enum.Parse(typeof(UxrBlendPoseType), GetBlendPoseTypes()[blendPoseIndex]) : UxrBlendPoseType.None;
                    ...
                    var newBlendValue = EditorGUILayout.Slider(new GUIContent("Preview Blend", "Previews in the scene window how the hand transitions from an open grip to a closed grip using the blend parameter"),
                        _blendValue,
                        0.0f,
                        1.0f,
                        GUILayout.Width(FieldWidth));
                ...
                var isNewNameAvailable = _currentHandPose != null && !string.IsNullOrEmpty(_poseName) && _poseName != _currentHandPose.name;
                var isNewNameValid = isNewNameAvailable && _poseName.IndexOfAny(Path.GetInvalidFileNameChars()) < 0 && _avatar.GetHandPose(_poseName, false) == null;
            ...
            if (_foldoutHands)
            {
                ...
                var leftHandTexturePos = new Vector2(HandMarginHorizontal, lastY + HandMarginVertical / 2);
                var rightHandTexturePos = new Vector2(HandMarginHorizontal * 2 + _rightHandTex.width, lastY + HandMarginVertical / 2);
                ...
                if (GUILayout.Button(new GUIContent("Copy Left To Right >", "Mirrors the current left hand's finger transforms to the right hand"), GUILayout.Width(ButtonWidth)))
                {
                    ...
                    var newHandDescriptor = GetHandDescriptor(_currentHandPose, CurrentPoseType, CurrentBlendPoseType, UxrHandSide.Left).Mirrored();
                ...
                if (GUILayout.Button(new GUIContent("< Copy Right To Left", "Mirrors the current right hand's finger transforms to the left hand"), GUILayout.Width(ButtonWidth)))
                {
                    ...
                    AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(_currentHandPose));
                    // Mirror hand descriptor
                    var newHandDescriptor = GetHandDescriptor(_currentHandPose, CurrentPoseType, CurrentBlendPoseType, UxrHandSide.Right).Mirrored();
                    // Recompute relative matrices
                ...
                lastY = (int) GUILayoutUtility.GetLastRect().yMax;
                var buttonsStartY = lastY;

                var resetButtonString = "Reset...";
                ...
                if (CurrentPoseType == UxrHandPoseType.Blend && _currentHandPose != null)
                {
                    ...
                    if (GUI.Button(GetLeftHandButtonRect(lastY), new GUIContent("Copy Fixed Pose To Open Grip", "Copies the left hand fingers from the fixed pose to the open grip")))
                    {
                        ...
                        SavePose(_currentHandPose, UxrHandSide.Left, CurrentPoseType, CurrentBlendPoseType, SaveHandPoseFlags.HandDescriptors);
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(_currentHandPose));
                        var newHandDescriptor = GetHandDescriptor(_currentHandPose, UxrHandPoseType.Fixed, UxrBlendPoseType.None, UxrHandSide.Left);
                    ...
                    if (GUI.Button(GetRightHandButtonRect(lastY), new GUIContent("Copy Fixed Pose To Open Grip", "Copies the right hand fingers from the fixed pose to the open grip")))
                    {
                        ...
                        SavePose(_currentHandPose, UxrHandSide.Right, CurrentPoseType, CurrentBlendPoseType, SaveHandPoseFlags.HandDescriptors);
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(_currentHandPose));
                        var newHandDescriptor = GetHandDescriptor(_currentHandPose, UxrHandPoseType.Fixed, UxrBlendPoseType.None, UxrHandSide.Right);
                    ...
                    if (GUI.Button(GetLeftHandButtonRect(lastY), new GUIContent("Copy Open Grip To Closed Grip", "Copies the left hand fingers from the open grip to the closed grip")))
                    {
                        ...
                        SavePose(_currentHandPose, UxrHandSide.Left, CurrentPoseType, CurrentBlendPoseType, SaveHandPoseFlags.HandDescriptors);
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(_currentHandPose));
                        var newHandDescriptor = GetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.OpenGrip, UxrHandSide.Left);
                        SetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.ClosedGrip, UxrHandSide.Left, newHandDescriptor);
                        UxrAvatarRig.UpdateHandUsingDescriptor(_avatar, UxrHandSide.Left, newHandDescriptor);
                        SavePose(_currentHandPose, UxrHandSide.Left, CurrentPoseType, UxrBlendPoseType.ClosedGrip, SaveHandPoseFlags.HandDescriptors);
                    ...
                    if (GUI.Button(GetRightHandButtonRect(lastY), new GUIContent("Copy Open Grip To Closed Grip", "Copies the right hand fingers from the open grip to the closed grip")))
                    {
                        ...
                        SavePose(_currentHandPose, UxrHandSide.Right, CurrentPoseType, CurrentBlendPoseType, SaveHandPoseFlags.HandDescriptors);
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(_currentHandPose));
                        var newHandDescriptor = GetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.OpenGrip, UxrHandSide.Right);
                        SetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.ClosedGrip, UxrHandSide.Right, newHandDescriptor);
                        UxrAvatarRig.UpdateHandUsingDescriptor(_avatar, UxrHandSide.Right, newHandDescriptor);
                        SavePose(_currentHandPose, UxrHandSide.Right, CurrentPoseType, UxrBlendPoseType.ClosedGrip, SaveHandPoseFlags.HandDescriptors);
                    ...
                    if (GUI.Button(GetLeftHandButtonRect(lastY), new GUIContent("Copy Closed Grip To Open Grip", "Copies the left hand fingers from the closed grip to the open grip")))
                    {
                        ...
                        SavePose(_currentHandPose, UxrHandSide.Left, CurrentPoseType, CurrentBlendPoseType, SaveHandPoseFlags.HandDescriptors);
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(_currentHandPose));
                        var newHandDescriptor = GetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.ClosedGrip, UxrHandSide.Left);
                        SetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.OpenGrip, UxrHandSide.Left, newHandDescriptor);
                        UxrAvatarRig.UpdateHandUsingDescriptor(_avatar, UxrHandSide.Left, newHandDescriptor);
                    ...
                    if (GUI.Button(GetRightHandButtonRect(lastY), new GUIContent("Copy Closed Grip To Open Grip", "Copies the right hand fingers from the closed grip to the open grip")))
                    {
                        ...
                        SavePose(_currentHandPose, UxrHandSide.Right, CurrentPoseType, CurrentBlendPoseType, SaveHandPoseFlags.HandDescriptors);
                        AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(_currentHandPose));
                        var newHandDescriptor = GetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.ClosedGrip, UxrHandSide.Right);
                        SetHandDescriptor(_currentHandPose, CurrentPoseType, UxrBlendPoseType.OpenGrip, UxrHandSide.Right, newHandDescriptor);
                        UxrAvatarRig.UpdateHandUsingDescriptor(_avatar, UxrHandSide.Right, newHandDescriptor);
...
*/
            ...
            lastY = (int) GUILayoutUtility.GetLastRect().yMax;
            var minWindowHeight = lastY;

            // Presets ////////////////////////////////////

            var rightAreaX = HandMarginHorizontal * 2 + _rightHandTex.width * 2 + HandMarginHorizontal + 10;
            var areaPresetsRect = new Rect(rightAreaX, 4, position.width - rightAreaX, position.height);
            ...
            if (_foldoutPresets)
            {
                var presetMargin = 10;
                var presetWidth = 200;
                var presetHeight = 140;
                var columns = Mathf.Max(1, (int) (areaPresetsRect.width - EditorGUIUtility.singleLineHeight - presetMargin) / (presetMargin + presetWidth));
                ...
                if (_handPosePresets != null)
                {
                    ...
                    for (var y = 0; finished == false; ++y)
                    {
                        ...
                        for (var x = 0; x < columns; ++x)
                        {
                            ...
                            var buttonNormal = GUI.skin.button.normal.background;
                            var buttonHover = GUI.skin.button.hover.background;
                            var buttonActive = GUI.skin.button.active.background;
                            var buttonFocused = GUI.skin.button.focused.background;
                            ...
                            var poseSuffix = "";
                            var tooltip = $"Load {_handPosePresets[index].Pose.name} pose";
            ...
            if (_updateHandTransforms && _poseIndex >= 0 && _poseIndex < _currentPoseNames.Count)
            {
                ...
                foreach (var skin in _avatar.GetAllAvatarRendererComponents())
                {
        ...
        private List<string> GetPoseTypes()
        {
            ...
            var types = Enum.GetNames(typeof(UxrHandPoseType));

            foreach (var poseType in types)
            {
        ...
        {
            var listTypes = new List<string>();
            var types = Enum.GetNames(typeof(UxrBlendPoseType));

            foreach (var blendPoseType in types)
            {
        ...
        private void SetPoseAsDefault(UxrAvatar avatar, string poseName)
        {
            if (avatar != null)
            {
                var serializedObject = new SerializedObject(avatar.GetAvatarPrefab());
                serializedObject.Update();
                var defaultHandPose = serializedObject.FindProperty(UxrAvatarEditor.PropertyDefaultHandPose);
        ...
        {
            var avatarPrefab = avatar.GetAvatarPrefab();
        ...
        private bool IsCurrentHandDifferentFromPose(UxrHandPoseAsset handPose, UxrHandSide handSide)
        {
            ...
            var handDescriptorTmp = new UxrHandDescriptor(_avatar, handSide);
            ...
            switch (handPose.PoseType)
            {
                ...
                {
                    var handDescriptor = handPose.GetHandDescriptor(handSide);
                ...
                {
                    var handDescriptorOpen = handPose.GetHandDescriptor(handSide, UxrBlendPoseType.OpenGrip);
                    var handDescriptorClosed = handPose.GetHandDescriptor(handSide, UxrBlendPoseType.ClosedGrip);
        ...
        private bool IsCurrentHandDifferentFromPose(UxrHandPoseAsset handPose, UxrHandSide handSide, UxrBlendPoseType blendPoseType)
        {
            ...
            var handDescriptor = handPose.GetHandDescriptor(handSide, blendPoseType);
            var handDescriptorTmp = new UxrHandDescriptor(_avatar, handSide);
        ...
        private bool CheckWarnSaveHandPose(UxrHandPoseAsset handPose, SaveHandPoseFlags saveFlags)
        {
            ...
            var isReadOnlyPose = _avatar && _avatar.GetHandPose(handPose.name, false) != handPose;
            var hasUnsavedChanges = IsCurrentHandDifferentFromPose(handPose, UxrHandSide.Left) || IsCurrentHandDifferentFromPose(handPose, UxrHandSide.Right);

            if (!isReadOnlyPose && hasUnsavedChanges)
            {
                var option = EditorUtility.DisplayDialogComplex("Unsaved Changes",
        ...
        {
            var isReadOnlyPose = _avatar && handPose && _avatar.GetHandPose(handPose.name, false) != handPose;
        ...
        private void SavePose(UxrHandPoseAsset handPose, UxrHandSide handSide, UxrHandPoseType poseType, UxrBlendPoseType blendPoseType, SaveHandPoseFlags saveFlags)
        {
            ...
            var handDescriptorDst = handPose.GetHandDescriptor(handSide, poseType, blendPoseType);
        ...
        private bool RenamePose(UxrAvatar avatar, UxrHandPoseAsset handPose, string newPoseName)
        {
            ...
            var errorMessage = AssetDatabase.RenameAsset(GetPoseAssetPath(handPose), newPoseName);
        ...
        private void DeletePose(UxrAvatar avatar, string poseName, bool deleteAsset)
        {
            ...
            var handPoseAsset = avatar.GetHandPose(poseName, false);

            if (handPoseAsset != null)
            {
                var serializedObject = new SerializedObject(avatar.GetAvatarPrefab());
                var propertyHandPoses = serializedObject.FindProperty(UxrAvatarEditor.PropertyHandPoses);
                ...
                var assetPath = GetPoseAssetPath(handPoseAsset);
        ...
        private UxrHandPoseAsset CreatePose(UxrAvatar avatar, bool reset = true, string path = null)
        {
            ...
            if (!string.IsNullOrEmpty(path))
            {
                ...
                var parentPrefab = avatar.GetParentPrefab(poseName);
                ...
                var handPose = CreateInstance<UxrHandPoseAsset>();
                ...
                var file = UxrEditorUtils.GetProjectRelativePath(path);
                ...
                var serializedObject = new SerializedObject(avatar.GetAvatarPrefab());
                serializedObject.Update();
                var handPoses = serializedObject.FindProperty(UxrAvatarEditor.PropertyHandPoses);
        ...
        {
            var newHandPoseAsset = CreatePose(avatar, false);
        ...
        private void PromptLoadExternalPose(bool loadLeft, bool loadRight)
        {
            ...
            if (!string.IsNullOrEmpty(path))
            {
                ...
                var externalPose = AssetDatabase.LoadAssetAtPath<UxrHandPoseAsset>(path);
        ...
        {
            var tryFindDefault = GetAvatarPoseNames(avatar).Count == 0;

            var path = EditorUtility.SaveFolderPanel("Pose Assets Destination Folder", CurrentSaveFolder, string.Empty);
            ...
            foreach (var preset in _handPosePresets)
            {
                var newHandPoseAsset = CreatePose(avatar, false, path + "/" + preset.Pose.name + ".asset");
        ...
        private void UpdateHandTransforms(UxrAvatar avatar, UxrHandPoseAsset handPose, float blend)
        {
            ...
            var active = avatar.gameObject.activeSelf;
        ...
        private void RefreshHandPosePresets()
        {
            try
            {
                ...
                var files = UxrEditorUtils.GetHandPosePresetFiles();
        ...
        {
            var leftHandAxes = _avatar.AvatarRigInfo.GetArmInfo(UxrHandSide.Left).HandUniversalLocalAxes;
            var rightHandAxes = _avatar.AvatarRigInfo.GetArmInfo(UxrHandSide.Right).HandUniversalLocalAxes;
            var leftFingerAxes = _avatar.AvatarRigInfo.GetArmInfo(UxrHandSide.Left).FingerUniversalLocalAxes;
            var rightFingerAxes = _avatar.AvatarRigInfo.GetArmInfo(UxrHandSide.Right).FingerUniversalLocalAxes;
            var leftHand = _avatar.LeftHand;
            var rightHand = _avatar.RightHand;
            var leftHandBone = _avatar.LeftHandBone;
            var rightHandBone = _avatar.RightHandBone;
            var typeSpread = UxrFingerAngleType.Spread;
            var typeCurl = UxrFingerAngleType.Curl;
        ...
        private void HandleFingerSpinners(Vector2 leftHandTexturePos, Vector2 rightHandTexturePos, bool selectGameObjectOnClick, UxrHandPoseType poseType, UxrBlendPoseType blendPoseType)
        {
            ...
            {
                foreach (var spinner in _fingerSpinners)
                {
                    var mouseRelativePos = Event.current.mousePosition - (spinner.HandSide == UxrHandSide.Left ? leftHandTexturePos : rightHandTexturePos);
            ...
            else if (Event.current.type == EventType.MouseDrag && _selectedFingerSpinner != null)
            {
                if (_selectedFingerSpinner != null)
                {
                    var mouseRelativePos = Event.current.mousePosition - (_selectedFingerSpinner.HandSide == UxrHandSide.Left ? leftHandTexturePos : rightHandTexturePos);

                    Undo.RecordObject(_selectedFingerSpinner.Target, _selectedFingerSpinner.Target.name + " Rotation");
                    var mouseOffset = mouseRelativePos - _selectedFingerSpinnerMouseStart;
                    var offset = _selectedFingerSpinner.Angle == UxrFingerAngleType.Spread ? mouseOffset.x : -mouseOffset.y;
            ...
            foreach (var spinner in _fingerSpinners)
            {
                var texturePos = spinner.HandSide == UxrHandSide.Left ? leftHandTexturePos : rightHandTexturePos;
                ...
                var halfSpinnerWidth = (int) (spinnerWidth * 0.5f);
                var halfSpinnerHeight = (int) (spinnerHeight * 0.5f);

                var spinnerRect = new Rect(spinner.MouseRect.center.x - halfSpinnerWidth + texturePos.x, spinner.MouseRect.center.y - halfSpinnerHeight + texturePos.y, spinnerWidth, spinnerHeight);
                var texCoords = new Rect(spinner.Angle == UxrFingerAngleType.Spread ? spinner.Offset * -SpinnerSpeed : 0.0f, spinner.Angle == UxrFingerAngleType.Curl ? spinner.Offset * -SpinnerSpeed : 0.0f, 1.0f, 1.0f);
                ...
                {
                    var leftArrowRect = new Rect(spinner.MouseRect.center.x - halfSpinnerWidth - _spinnerArrowLeftTex.width - arrowMargin + texturePos.x,
                    ...
                    var rightArrowRect = new Rect(spinner.MouseRect.center.x + halfSpinnerWidth + arrowMargin + texturePos.x,
                ...
                {
                    var upArrowRect = new Rect(spinner.MouseRect.center.x - (int) (_spinnerArrowUpTex.width / 2.0f) + texturePos.x,
                    ...
                    var downArrowRect = new Rect(spinner.MouseRect.center.x - (int) (_spinnerArrowDownTex.width / 2.0f) + texturePos.x,
            ...
            if (_selectedFingerSpinner == null)
            {
                ...
                {
                    var texturePos = spinner.HandSide == UxrHandSide.Left ? leftHandTexturePos : rightHandTexturePos;
                    var mouseRect = new Rect(spinner.MouseRect.x + texturePos.x, spinner.MouseRect.y + texturePos.y, spinner.MouseRect.width, spinner.MouseRect.height);
        ...
        private List<string> GetFormattedAvatarPoseNames(IEnumerable<string> poseNames, string defaultPoseName)
        {
            ...
            var formattedPoseNames = new List<string>(poseNames.Count());
        ...
        {
            var grabbableObjectSelected = Selection.objects != null && Selection.objects.Length == 1 && Selection.objects[0].GetType() == typeof(GameObject)
            ...
            {
                var grabbers = _avatar.GetComponentsInChildren<UxrGrabber>();
                ...
                for (var i = 0; i < grabbers.Length; ++i)
                {
                    if (grabbers[i].Side == UxrHandSide.Left)
                    {
                        ...
                        {
                            var snapObject = new GameObject(poseName + (handSide == UxrHandSide.Left ? "Left" : "Right"));
                            var alignTransformComponent = snapObject.AddComponent<UxrGrabbableObjectSnapTransform>();
        ...
        private Rect GetPosesMenuButtonRect(int column, int y)
        {
            ...
            var columnWidth = totalWidth / PosesMenuTotalColumns;
            var posMiddle = columnWidth * column + columnWidth * 0.5f;
            var posButton = posMiddle - ButtonWidth * 0.5f;
        ...
        {
            var rightHandButtonPos = HandMarginHorizontal * 2 + _leftHandTex.width * 1.5f - ButtonWidth / 2;
