...
namespace UltimateXR.Editor.Sdks
{
    ...
    public static partial class UxrSdkManager
    {
        ...
        {
            var targetGroupNames = Enum.GetNames(typeof(BuildTargetGroup));
            ...
            foreach (BuildTargetGroup targetGroup in Enum.GetValues(typeof(BuildTargetGroup)))
            {
                ...
                var targetGroupName = targetGroupNames[targetGroupIndex];
                targetGroupIndex++;
                ...
                var currentSymbols = new List<string>(PlayerSettings.GetScriptingDefineSymbolsForGroup(targetGroup).Split(';'));

                for (var currentSymbolIndex = 0; currentSymbolIndex < currentSymbols.Count; ++currentSymbolIndex)
                {
                ...
                var availableSymbols = new List<string>(locator.AvailableSymbols);

                foreach (var sdkSymbolString in locator.AllSymbols)
                {
        ...
        public static void RegisterLocator(UxrSdkLocator locator)
        {
            ...
            var locatorAlreadyRegistered = false;

            foreach (var registeredLocator in s_locators)
            {
        ...
        public static bool IsAvailable<T>() where T : UxrSdkLocator
        {
            if (s_locators != null)
            {
                foreach (var locator in s_locators)
                {
