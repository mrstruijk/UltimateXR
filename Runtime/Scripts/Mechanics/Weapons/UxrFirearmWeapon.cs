    ...
    public class UxrFirearmWeapon : UxrWeapon
    {
            ...
            foreach (var trigger in _triggers)
            {
                ...
                if (trigger.AmmunitionMagAnchor != null)
                {
                    if (trigger.AmmunitionMagAnchor.CurrentPlacedObject != null)
                    {
                        ...
                        var magCollider = trigger.AmmunitionMagAnchor.CurrentPlacedObject.GetComponentInChildren<Collider>();
        ...
        private void UxrManager_AvatarsUpdated()
        {
            foreach (var trigger in _triggers)
            {
                ...
                if (trigger.TriggerGrabbable && UxrGrabManager.Instance.GetGrabbingHand(trigger.TriggerGrabbable, trigger.GrabbableGrabPointIndex, out UxrGrabber grabber))
                {
                    ...
                    var triggerPressAmount = UxrAvatar.LocalAvatarInput.GetInput1D(grabber.Side, UxrInput1D.Trigger);
                    ...
                    var triggerPressed = UxrAvatar.LocalAvatarInput.GetButtonsPress(grabber.Side, UxrInputButtons.Trigger);
                    var triggerPressDown = UxrAvatar.LocalAvatarInput.GetButtonsPressDown(grabber.Side, UxrInputButtons.Trigger);
                    var shoot = false;
        ...
        private void MagTarget_Removed(object sender, UxrManipulationEventArgs e)
        {
            foreach (var trigger in _triggers)
            {
                ...
                {
                    var magCollider = e.GrabbableObject.GetComponentInChildren<Collider>();
        ...
        private void RootGrabbable_ConstraintsApplied(object sender, UxrApplyConstraintsEventArgs e)
        {
            ...
            var grabbableObject = sender as UxrGrabbableObject;
            var grabbableTransform = grabbableObject.transform;
            var grabbingHandCount = UxrGrabManager.Instance.GetGrabbingHandCount(grabbableObject);

            foreach (var trigger in _triggers)
            {
                if (trigger.RecoilTimer > 0.0f)
                {
                    var recoilT = trigger.RecoilDurationSeconds > 0.0f ? (trigger.RecoilDurationSeconds - trigger.RecoilTimer) / trigger.RecoilDurationSeconds : 0.0f;

                    var recoilRight = _recoilAxes != null ? _recoilAxes.right : grabbableTransform.right;
                    var recoilUp = _recoilAxes != null ? _recoilAxes.up : grabbableTransform.up;
                    var recoilForward = _recoilAxes != null ? _recoilAxes.forward : grabbableTransform.forward;
                    var recoilPosition = _recoilAxes != null ? _recoilAxes.position : grabbableTransform.position;

                    var amplitude = 1.0f - recoilT;
                    var recoilOffset = grabbingHandCount == 1 ? amplitude * trigger.RecoilOffsetOneHand : amplitude * trigger.RecoilOffsetTwoHands;
        ...
        private static int GetAmmoCapacity(UxrFirearmTrigger trigger)
        {
            ...
            {
                var mag = trigger.AmmunitionMagAnchor.CurrentPlacedObject.GetCachedComponent<UxrFirearmMag>();
        ...
        private bool TryToShootRound(UxrFirearmTrigger trigger)
        {
            if (GetAmmoLeft(trigger) > 0 && trigger.LastShotTimer <= 0.0f)
            {
                ...
                var triggerIndex = _triggers.IndexOf(trigger);
        ...
        private UxrGrabbableObject RootGrabbable
        {
            get
            {
                var firstTriggerGrabbable = _triggers.Count > 0 && _triggers[0].TriggerGrabbable != null ? _triggers[0].TriggerGrabbable : null;
                ...
                if (firstTriggerGrabbable)
                {
                    ...
                    var weaponRootGrabbableTransform = firstTriggerGrabbable.UsesGrabbableParentDependency ? firstTriggerGrabbable.GrabbableParentDependency : firstTriggerGrabbable.transform;
                    var rootGrabbable = weaponRootGrabbableTransform.GetComponent<UxrGrabbableObject>();
