    ...
    public partial class UxrWeaponManager : UxrSingleton<UxrWeaponManager>, IUxrLogger
    {
        ...
        private void UpdateProjectiles()
        {
            for (var i = 0; i < _projectiles.Count; ++i)
            {
                ...
                var oldPos = _projectiles[i].ProjectileLastPosition;
                var newPos = _projectiles[i].Projectile.transform.position;

                var projectileForward = _projectiles[i].Projectile.transform.forward;
                ...
                if (_projectiles[i].ShotDescriptor.UseAutomaticProjectileTrajectory)
                {
                    var rayLength = Vector3.Distance(oldPos, newPos + projectileForward * _projectiles[i].ShotDescriptor.ProjectileLength);

                    if (Physics.Raycast(oldPos, projectileForward, out var raycastHit, rayLength, _projectiles[i].ShotLayerMask, QueryTriggerInteraction.Ignore))
                    {
                        var projectileDeflect = raycastHit.collider.GetComponentInParent<UxrProjectileDeflect>();

                        // Has UxrProjectileDeflect component?

                        if (projectileDeflect != null)
                        {
                            var newForward = Vector3.Reflect(projectileForward, raycastHit.normal).normalized;
                        ...
                        {
                            var targetActor = raycastHit.collider.GetComponentInParent<UxrActor>();

                            if (targetActor != null)
                            {
                                // Impact with an actor.

                                var normalizedDistance = _projectiles[i].ProjectileDistanceTravelled / _projectiles[i].ShotDescriptor.ProjectileMaxDistance;
                                var damage = Mathf.Lerp(_projectiles[i].ShotDescriptor.ProjectileDamageNear, _projectiles[i].ShotDescriptor.ProjectileDamageFar, normalizedDistance);
                            ...
                            else
                            {
                                ...
                                {
                                    var instantiateForward = Vector3.Reflect(projectileForward, raycastHit.normal).normalized;
                                    var newInstance = Instantiate(_projectiles[i].ShotDescriptor.PrefabInstantiateOnImpact, raycastHit.point, Quaternion.LookRotation(instantiateForward));
                                ...
                                var rigidbody = raycastHit.collider.GetComponentInParent<Rigidbody>();
                                ...
                                var overrideDecal = raycastHit.collider.GetComponentInParent<UxrOverrideImpactDecal>();
        ...
        {
            foreach (var damageActorPair in _damageActors)
            {
                var distance = Vector3.Distance(damageActorPair.Value.Transform.position, position);

                if (distance < radius)
                {
                    var damage = Mathf.Lerp(nearDamage, farDamage, distance / radius);
        ...
        private void OnDamageReceived(UxrDamageEventArgs e)
        {
            ...
            {
                var sourceInfo = e.ActorSource != null ? $" from actor {e.ActorSource.name}." : string.Empty;
        ...
        private readonly Dictionary<UxrActor, ActorInfo> _damageActors = new();
        private readonly List<ProjectileInfo> _projectiles = new();
