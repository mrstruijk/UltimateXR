    ...
    public partial class WristConnectionRays : UxrComponent
    {
        ...
        private void UpdateRays(Vector3 src, Vector3 dst)
        {
            foreach (var ray in _rays)
            {
                ...
                var rayLength = Vector3.Distance(src, dst) / ray.LineRenderer.transform.lossyScale.z;

                Vector3[] positions =
                {
                    new(0.0f, 0.0f, 0.0f),
                    new(0.0f, 0.0f, rayLength * _gradientPosStart1),
                    new(0.0f, 0.0f, rayLength * _gradientPosStart2),
                    new(0.0f, 0.0f, rayLength * _gradientPosEnd1),
                    new(0.0f, 0.0f, rayLength * _gradientPosEnd2),
                    new(0.0f, 0.0f, rayLength)
                };

                var offset = (ray.GameObject.transform.right * ray.OffsetXY.x + ray.GameObject.transform.up * ray.OffsetXY.y).normalized * ray.Offset;
