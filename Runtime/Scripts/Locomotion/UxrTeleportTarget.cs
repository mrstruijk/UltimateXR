...
namespace UltimateXR.Locomotion
{
    ...
    public class UxrTeleportTarget : UxrComponent
    {
        ...
        protected override void Awake()
        {
            ...
            foreach (var pSystemRenderer in _particleSystemRenderers)
            {
        ...
        public void SetMaterialColor(Color color)
        {
            foreach (var targetRenderer in _targetRenderers)
            {
                if (_object2MaterialID.TryGetValue(targetRenderer.gameObject, out var teleportMaterialID))
                {
            ...
            foreach (var pSystem in _particleSystems)
            {
                ...
                {
                    var keys = colorOverLifetime.color.gradient.colorKeys;
            ...
            foreach (var pSystemRenderer in _particleSystemRenderers)
            {
                if (_object2MaterialID.TryGetValue(pSystemRenderer.gameObject, out var teleportMaterialID))
                {
                    var materials = pSystemRenderer.materials;
