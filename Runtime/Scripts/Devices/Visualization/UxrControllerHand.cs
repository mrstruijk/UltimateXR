...
namespace UltimateXR.Devices.Visualization
{
    ...
    public partial class UxrControllerHand : UxrAvatarComponent<UxrControllerHand>
    {
        ...
        {
            var runtimeHandPose = avatar.GetCurrentRuntimeHandPose(handSide);
        ...
        public void UpdateFinger(UxrFingerType finger, UxrFingerContactInfo fingerContactInfo)
        {
            ...
            if (_fingers.TryGetValue(finger, out var fingerInfo))
            {
                ...
                var fingerIKParent = fingerInfo.FingerIKSolver.Links[0].Bone.parent;
                ...
                if (fingerInfo.TimerToGoal > 0.0f)
                {
                    ...
                    var t = 1.0f - Mathf.Clamp01(fingerInfo.TimerToGoal / fingerInfo.FingerToGoalDuration);
        ...
        {
            foreach (var fingerPair in _fingers)
            {
