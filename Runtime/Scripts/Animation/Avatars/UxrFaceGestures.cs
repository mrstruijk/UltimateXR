...
namespace UltimateXR.Animation.Avatars
{
    ...
    public class UxrFaceGestures : UxrComponent
    {
        ...
        {
            float maxLevel = 0;
            var waveData = new float[MicrophoneSampleWindow];
            var micPosition = Microphone.GetPosition(null) - (MicrophoneSampleWindow + 1);
            ...
            for (var i = 0; i < MicrophoneSampleWindow; ++i)
            {
        ...
        private IEnumerator BlinkCoroutine()
        {
            while (true)
            {
                ...
                var blinkDuration = Random.Range(_eyeBlinkDurationMin, _eyeBlinkDurationMax);
                ...
                {
                    var t = Mathf.Clamp01((Time.time - startTime) / (blinkDuration * 0.5f));

                    var rotationTop = Quaternion.AngleAxis(_eyeBlinkTopClosedAngle * t, _eyeBlinkTopLocalAxis);
                    var rotationBottom = Quaternion.AngleAxis(_eyeBlinkBottomClosedAngle * t, _eyeBlinkBottomLocalAxis);
                ...
                var rotationClosedTop = Quaternion.AngleAxis(_eyeBlinkTopClosedAngle, _eyeBlinkTopLocalAxis);
                var rotationClosedBottom = Quaternion.AngleAxis(_eyeBlinkBottomClosedAngle, _eyeBlinkBottomLocalAxis);
                ...
                while (Time.time - startTime < blinkDuration * 0.5f)
                {
                    var t = 1.0f - Mathf.Clamp01((Time.time - startTime) / (blinkDuration * 0.5f));
        ...
        private IEnumerator SwitchLookCoroutine()
        {
            while (true)
            {
                ...
                var switchDuration = Random.Range(_eyeSwitchLookDurationMin, _eyeSwitchLookDurationMax);
                var startTime = Time.time;
                ...
                var rotLeft = _eyeLeft.localRotation;
                var rotRight = _eyeRight.localRotation;

                while (Time.time - startTime < switchDuration)
                {
                    var t = Mathf.Clamp01((Time.time - startTime) / switchDuration);
