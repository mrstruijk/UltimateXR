...
namespace UltimateXR.Animation
{
    ...
    public abstract class UxrAnimatedComponent<T> : UxrComponent where T : UxrAnimatedComponent<T>
    {
        ...
        {
            var anim = gameObject.GetComponent<T>();
        ...
        private void Update()
        {
            ...
            {
                var value = UxrInterpolator.Interpolate(_valueStart, _valueEnd, AnimationTime, InterpolationSettings);
                SetParameterValue(value);
            ...
            else if (AnimationMode == UxrAnimationMode.Noise)
            {
                ...
                {
                    var tX = Mathf.PerlinNoise(_valueNoiseOffset[0] + AnimationTime * _valueNoiseFrequency[0], _valueNoiseOffset[0]);
                    var tY = Mathf.PerlinNoise(_valueNoiseOffset[1] + AnimationTime * _valueNoiseFrequency[1], _valueNoiseOffset[1]);
                    var tZ = Mathf.PerlinNoise(_valueNoiseOffset[2] + AnimationTime * _valueNoiseFrequency[2], _valueNoiseOffset[2]);
                    var tW = Mathf.PerlinNoise(_valueNoiseOffset[3] + AnimationTime * _valueNoiseFrequency[3], _valueNoiseOffset[3]);
