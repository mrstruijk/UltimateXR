...
namespace UltimateXR.Extensions.Unity.Math
{
    ...
    public static class Matrix4x4Ext
    {
        ...
        {
            var position = Vector3.Lerp(matrixA.MultiplyPoint(Vector3.zero), matrixB.MultiplyPoint(Vector3.zero), blendValue);
            var rotation = Quaternion.Slerp(matrixA.rotation, matrixB.rotation, blendValue);
            var scale = Vector3.Lerp(matrixA.lossyScale, matrixB.lossyScale, blendValue);
        ...
        public static Matrix4x4 GetObliqueMatrix(this Matrix4x4 projection, Vector4 clipPlane)
        {
            ...
            var q = projection.inverse * new Vector4(Mathf.Sign(clipPlane.x), Mathf.Sign(clipPlane.y), 1.0f, 1.0f);
