...
namespace UltimateXR.UI.UnityInputModule
{
    ...
    public class UxrLaserPointerRaycaster : UxrGraphicRaycaster
    {
        ...
        public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList)
        {
            ...
            var pointerEventData = eventData as UxrPointerEventData;
            ...
            Raycast(_canvas, pointerEventData.LaserPointer, eventCamera, ray, ref _raycastResults, ref resultAppendList, out var _);
        ...
        private void Raycast(Canvas canvas, UxrLaserPointer laserPointer, Camera cam, Ray ray, ref List<RaycastResult> results, ref List<RaycastResult> resultAppendList, out float occluderDistance)
        {
            ...
            {
                var rectTransformCanvas = canvas.GetComponent<RectTransform>();
                ...
                var blocking3D = laserPointer != null ? laserPointer.TargetTypes.HasFlag(UxrLaserPointerTargetTypes.Colliders3D) : blockingObjects.HasFlag(BlockingObjects.ThreeD);
                ...
                var blocking2D = laserPointer != null ? laserPointer.TargetTypes.HasFlag(UxrLaserPointerTargetTypes.Colliders2D) : blockingObjects.HasFlag(BlockingObjects.TwoD);

                if (blocking2D)
                {
                    var hit = Physics2D.Raycast(ray.origin, ray.direction, maxDistance, laserPointer != null ? laserPointer.BlockingMask : m_BlockingMask);
            ...
            var processUI = laserPointer == null || laserPointer.TargetTypes.HasFlag(UxrLaserPointerTargetTypes.UI);

            if (processUI)
            {
                var listGraphics = GraphicRegistry.GetGraphicsForCanvas(canvas);

                for (var i = 0; i < listGraphics.Count; i++)
                {
                    var graphic = listGraphics[i];
                    ...
                    var distance = Vector3.Dot(graphic.transform.forward, graphic.transform.position - ray.origin) / Vector3.Dot(graphic.transform.forward, ray.direction);
