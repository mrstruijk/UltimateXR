    ...
    public class UxrFingerTipRaycaster : UxrGraphicRaycaster
    {
        ...
        private void Raycast(Canvas canvas, Camera cam, Ray ray, ref List<RaycastResult> results, ref List<RaycastResult> resultAppendList)
        {
            ...
            for (var i = 0; i < listGraphics.Count; ++i)
            {
                ...
                var distance = Vector3.Dot(listGraphics[i].transform.forward, listGraphics[i].transform.position - ray.origin) / Vector3.Dot(listGraphics[i].transform.forward, ray.direction);
