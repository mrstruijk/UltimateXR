...
namespace UltimateXR.Manipulation
{
    ...
    public class UxrGrabbableObject : UxrComponent<UxrGrabbableObject>, IUxrGrabbable
    {
        ...
        private void UxrManager_StageUpdated(UxrUpdateStage stage)
        {
            ...
            {
                for (var i = 0; i < GrabPointCount; ++i)
                {
                    var grabPointInfo = GetGrabPoint(i);

                    if (grabPointInfo.RuntimeGrabs != null)
                    {
                        foreach (var grabPairInfo in grabPointInfo.RuntimeGrabs)
                        {
        ...
        [SerializeField] private UxrGrabPointInfo _grabPoint = new();
        ...
        public int SingleRotationAxisIndex
        {
            get
            {
                ...
                var constrainedAxisIndex = 0;
                var constrainedAxisCount = 0;
        ...
        public float MinSingleRotationDegrees
        {
            get
            {
                var singleRotationAxisIndex = SingleRotationAxisIndex;
        ...
        {
            var grabPointInfo = GetGrabPoint(grabPoint);
            var grabberProximityTransform = grabber.GetProximityTransform(grabPointInfo.GrabberProximityTransformIndex);
            ...
            else
            {
                ...
                if (grabPointInfo.GrabProximityMode == UxrGrabProximityMode.UseProximity)
                {
                    ...
                    {
                        var relativeAngleDegrees = Quaternion.Angle(grabber.transform.rotation, GetGrabPointGrabAlignTransform(grabber.Avatar, grabPoint, grabber.Side).rotation);
            ...
            for (var otherGrabbedPoint = 0; otherGrabbedPoint < GrabPointCount; ++otherGrabbedPoint)
            {
        ...
        public bool CanBeGrabbedByGrabber(UxrGrabber grabber, int grabPoint)
        {
            ...
            var isBeingGrabbedByOtherPoint = GrabPointCount > 1 && UxrGrabManager.Instance.IsBeingGrabbed(this) && !UxrGrabManager.Instance.IsBeingGrabbed(this, grabPoint);
            var isBeingGrabbedBySameShape = _grabPointShapes.ContainsKey(grabPoint) && UxrGrabManager.Instance.IsBeingGrabbed(this, grabPoint);
            ...
            GetDistanceFromGrabber(grabber, grabPoint, out var distance, out var distanceWithoutRotation);
        ...
        public bool ComputeRequiredGrabberTransform(UxrGrabber grabber, int grabPoint, out Vector3 grabberPosition, out Quaternion grabberRotation)
        {
            ...
            var grabPointInfo = GetGrabPoint(grabPoint);
            var snapTransform = GetGrabPointGrabAlignTransform(grabber.Avatar, grabPoint, grabber.Side);
            ...
            {
                var controller3DModel = grabber != null && grabber.Avatar != null ? grabber.Avatar.ControllerInput.GetController3DModel(grabber.Side) : null;

                if (controller3DModel != null)
                {
                    var relativeTrackerRotation = Quaternion.Inverse(grabber.transform.rotation) * controller3DModel.ForwardTrackingRotation;
                    var trackerRotation = grabberRotation * relativeTrackerRotation;
                    var sourceAlignAxes = grabPointInfo.AlignToControllerAxes != null ? grabPointInfo.AlignToControllerAxes.rotation : transform.rotation;
        ...
        {
            return CanBePlacedOnAnchor(anchor, out var distance);
        }
        ...
        public void CheckAndApplyLockHands()
        {
            for (var i = 0; i < GrabPointCount; ++i)
            {
                if (UxrGrabManager.Instance.GetGrabbingHands(this, i, out var grabbers))
                {
        ...
        public void CheckAndApplyLockHand(UxrGrabber grabber, int grabPoint)
        {
            ...
            {
                if (!GetGrabPoint(grabPoint).RuntimeGrabs.TryGetValue(grabber, out var grabInfo))
                {
                    return;
                }
        ...
        public UxrAxis GetMostProbableLongitudinalRotationAxis()
        {
            ...
            var localBounds = gameObject.GetLocalBounds(true);
            var maxUncenteredComponent = -1;
            var maxUncenteredDistance = 0.0f;

            for (var i = 0; i < 3; ++i)
            {
                var centerOffset = Mathf.Abs(localBounds.center[i]);
        ...
        public UxrRotationProvider GetAutoRotationProvider(Vector3 gripPos)
        {
            ...
            var longitudinalAxis = GetMostProbableLongitudinalRotationAxis();
            var singleRotationAxisIndex = SingleRotationAxisIndex;
            var leverageDistance = 0.0f;
        ...
        {
            if (_grabPointShapes.TryGetValue(grabPoint, out var shape))
            {
                return shape;
            }
        ...
        {
            var gripPoseInfo = GetGrabPoint(grabPoint).GetGripPoseInfo(avatar);
        ...
        internal Quaternion GetGrabberControllerAlignmentRotation(UxrGrabber grabber, int grabPoint)
        {
            ...
            {
                var relativeTrackerRotation = Quaternion.Inverse(grabber.transform.rotation) * controller3DModel.ForwardTrackingRotation;
                var trackerRotation = grabber.transform.rotation * relativeTrackerRotation;
                var sourceAlignAxes = grabPointInfo.AlignToControllerAxes != null ? grabPointInfo.AlignToControllerAxes.rotation : transform.rotation;
                var grabberTargetRotation = sourceAlignAxes * Quaternion.Inverse(trackerRotation) * grabber.transform.rotation;
        ...
        internal void ComputeGrabTransforms(UxrGrabber grabber, int grabPoint)
        {
            ...
            if (grabPointInfo != null && grabPointInfo.RuntimeGrabs.TryGetValue(grabber, out var grabInfo))
            {
        ...
        internal void CheckAndApplyConstraints(UxrGrabber grabber, int grabPoint, Vector3 localPositionBeforeUpdate, Quaternion localRotationBeforeUpdate, bool propagateEvents)
        {
            ...
            var gripInfo = grabPointInfo.RuntimeGrabs[grabber];
            ...
            if (RotationProvider == UxrRotationProvider.HandPositionAroundPivot)
            {
                ...
                var rangeOfMotionAxisCount = RangeOfMotionRotationAxisCount;
                ...
                var grabDirection = grabber.transform.TransformPoint(gripInfo.GrabberLocalLeverageSource) - transform.position;
                var initialGrabDirection = TransformExt.GetWorldPosition(transform.parent, gripInfo.GrabberLocalParentLeverageSourceOnGrab) - transform.position;
                ...
                if (rangeOfMotionAxisCount == 1)
                {
                    ...
                    var projectedGrabDirection = Vector3.ProjectOnPlane(grabDirection, localRotationBeforeUpdate * rotationAxis);
                    var projectedInitialGrabDirection = Vector3.ProjectOnPlane(initialGrabDirection, localRotationBeforeUpdate * rotationAxis);

                    var angle = Vector3.SignedAngle(projectedInitialGrabDirection, projectedGrabDirection, localRotationBeforeUpdate * rotationAxis);
                    var angleDelta = angle - _singleRotationAngleGrab.ToEuler180();
                    ...
                    var rotationAngle = (_singleRotationAngleCumulative + _singleRotationAngleGrab).Clamped(_rotationAngleLimitsMin[rotationAxis], _rotationAngleLimitsMax[rotationAxis]);
                ...
                else
                {
                    ...
                    var rotUsingHandOrientation = grabber.transform.rotation * GetGrabPointRelativeGrabRotation(grabber, grabPoint);
                    ...
                    var rotationOnGrab = transform.GetParentRotation() * gripInfo.LocalRotationOnGrab;
                    var rotUsingHandPosAroundAxis = Quaternion.FromToRotation(initialGrabDirection.normalized, grabDirection.normalized) * rotationOnGrab;
                    var rotCorrection = Quaternion.FromToRotation(rotUsingHandOrientation * _rotationLongitudinalAxis, rotUsingHandPosAroundAxis * _rotationLongitudinalAxis);
                    var localRotation = Quaternion.Inverse(transform.GetParentRotation()) * rotCorrection * rotUsingHandOrientation;
            ...
            else
            {
                ...
                {
                    var sourcePosition = TransformExt.GetWorldPosition(grabPointInfo.RuntimeGrabs[grabber].GrabAlignParentTransformUsed, grabPointInfo.RuntimeGrabs[grabber].RelativeGrabAlignPosition);
                    var sourceRotation = TransformExt.GetWorldRotation(grabPointInfo.RuntimeGrabs[grabber].GrabAlignParentTransformUsed, grabPointInfo.RuntimeGrabs[grabber].RelativeGrabAlignRotation);
                    var targetRotation = grabber.transform.rotation;
                    ...
                    if (gripInfo.GrabTimer > 0.0f)
                    {
                        ...
                        var t = 1.0f - Mathf.Clamp01(gripInfo.GrabTimer / ObjectAlignmentSeconds);
                ...
                if (_constraintExitTimer > 0.0f)
                {
                    var constraintExitT = 1.0f - _constraintExitTimer / ConstrainSeconds;
        ...
        {
            var grabPointInfo = GetGrabPoint(grabPoint);
            var grabAlignTransform = GetGrabPointGrabAlignTransform(grabber.Avatar, grabPoint, grabber.Side);
            ...
            if (grabPointInfo.RuntimeGrabs.TryGetValue(grabber, out var grabInfo) == false)
            {
                grabInfo = new UxrRuntimeGripInfo();
                grabPointInfo.RuntimeGrabs.Add(grabber, grabInfo);
            }

            var snapPosition = grabAlignTransform.position;
            var snapRotation = grabAlignTransform.rotation;
            ...
            var snapMatrix = Matrix4x4.TRS(snapPosition, snapRotation, grabAlignTransform.lossyScale);
            var localProximity = grabAlignTransform.InverseTransformPoint(GetGrabPointGrabProximityTransform(grabber, grabPoint).position);
            ...
            if (RotationProvider == UxrRotationProvider.HandPositionAroundPivot && GetGrabPointSnapModeAffectsRotation(grabPoint))
            {
                ...
                var separation = UxrConstants.Hand.HandWidth;
                var distanceInner = Vector3.Distance(transform.position, snapPosition + snapRotation * grabber.LocalPalmThumbDirection * (separation * 0.5f));
                var distanceOuter = Vector3.Distance(transform.position, snapPosition - snapRotation * grabber.LocalPalmThumbDirection * (separation * 0.5f));
        ...
        protected override void Awake()
        {
            ...
            var fixedMin = Vector3.Min(_translationLimitsMin, _translationLimitsMax);
            var fixedMax = Vector3.Max(_translationLimitsMin, _translationLimitsMax);
            ...
            {
                var newAnchor = new GameObject($"{name} Auto Anchor", typeof(UxrGrabbableObjectAnchor)).GetComponent<UxrGrabbableObjectAnchor>();
            ...
            var grabPointShapes = GetComponents<UxrGrabPointShape>();

            foreach (var grabPointShape in grabPointShapes)
            {
        ...
        private Transform GetGrabbableParentDependency(Transform grabbableTransform)
        {
            ...
            {
                var parentGrabbableObject = grabbableTransform.parent.GetComponentInParent<UxrGrabbableObject>();
        ...
        private void ConstrainTransform(bool processPosition, bool processRotation, Vector3 unprocessedLocalPosition, Quaternion unprocessedLocalRotation)
        {
            ...
            {
                var targetLocalRotation = InitialLocalRotation;
            ...
            {
                var localPos = unprocessedLocalPosition;
                var targetLocalPos = InitialLocalPosition;
                ...
                else if (_translationConstraintMode == UxrTranslationConstraintMode.RestrictLocalOffset)
                {
                    if (transform.parent != null || InitialParent == null)
                    {
                        ...
                        var localPosOffset = InitialRelativeMatrix.inverse.MultiplyPoint3x4(transform.localPosition);
        ...
        private Quaternion ClampRotation(Quaternion rot, Quaternion rotBeforeUpdate, Quaternion initialRot, Vector3 eulerMin, Vector3 eulerMax, bool invertRotation, ref float singleRotationAngle)
        {
            ...
            if (rangeOfMotionAxisCount > 1)
            {
                var invertPitchYaw = false;
                var ignorePitchYaw = -1;
                var clampLongitudinal = false;

                // Use classic yaw/pitch clamping when more than one axis has constrained range of motion.

                var axis1 = RangeOfMotionRotationAxes.First();
                var axis2 = RangeOfMotionRotationAxes.Last();
                var longitudinalAxis = UxrAxis.OtherThan(axis1, axis2);
                ...
                var relativeRot = Quaternion.Inverse(initialRot) * rot;
                var targetDirection = relativeRot * longitudinalAxis;

                var pitch = -Mathf.Asin(targetDirection[axis2]) * Mathf.Rad2Deg;
                var yaw = Mathf.Atan2(targetDirection[axis1], targetDirection[longitudinalAxis]) * Mathf.Rad2Deg;
                ...
                var clampedRot = Quaternion.Euler(clampedEuler);

                // Clamp rotation around the longitudinal axis if necessary

                if (clampLongitudinal)
                {
                    var fixedLongitudinal = clampedRot * longitudinalAxis;
                    var correctionRot = Quaternion.FromToRotation(targetDirection, fixedLongitudinal);
                    var withRoll = correctionRot * relativeRot;
                    var roll = Vector3.SignedAngle(clampedRot * axis1, withRoll * axis1, fixedLongitudinal) * (invertRotation ? -1.0f : 1.0f);
                    var clampedRoll = roll.Clamped(eulerMin[longitudinalAxis], eulerMax[longitudinalAxis]);
            ...
            var singleAxisIndex = SingleRotationAxisIndex;
            Vector3 rotationAxis = (UxrAxis) singleAxisIndex;
            Vector3 perpendicularAxis = ((UxrAxis) singleAxisIndex).Perpendicular;
            var initialPerpendicularVector = initialRot * perpendicularAxis;
            var currentPerpendicularVector = Vector3.ProjectOnPlane(rot * perpendicularAxis, rotationAxis);
            var angle = Vector3.SignedAngle(initialPerpendicularVector, currentPerpendicularVector, rotationAxis) * (invertRotation ? -1.0f : 1.0f);
            var angleDelta = angle - singleRotationAngle.ToEuler180();
        ...
        private readonly Dictionary<int, bool> _grabPointEnabledStates = new();
        ...
        private Dictionary<int, UxrGrabPointShape> _grabPointShapes = new();
        ...
        public string Editor_GetSelectedAvatarPrefabGuidForGrips()
        {
            ...
            {
                var avatar = _selectedAvatarForGrips.GetComponent<UxrAvatar>();
        ...
        {
            var registeredAvatars = new List<string>();

            // All grab points have the same amount of avatar grip entries. Use default grab point.

            var grabPointInfo = GetGrabPoint(0);

            foreach (var poseInfo in grabPointInfo.AvatarGripPoseEntries)
            {
        ...
        public void Editor_CheckRegisterAvatarsInNewGrabPoints()
        {
            ...
            for (var i = 0; i < GrabPointCount; ++i)
            {
                ...
                foreach (var gripPoseInfo in grabPointInfo.AvatarGripPoseEntries)
                {
            ...
            for (var i = 0; i < GrabPointCount; ++i)
            {
                ...
                foreach (var prefabGuid in registeredAvatars)
                {
        ...
        public string Editor_GetGrabAlignTransformAvatar(Transform snapTransform)
        {
            for (var i = 0; i < GrabPointCount; ++i)
            {
                ...
                {
                    foreach (var gripPose in grabPointInfo.AvatarGripPoseEntries)
                    {
        ...
        public IEnumerable<Transform> Editor_GetAllRegisteredAvatarGrabAlignTransforms(UxrAvatar avatar, bool includeLeft, bool includeRight, bool usePrefabInheritance)
        {
            for (var i = 0; i < GrabPointCount; ++i)
            {
                ...
                if (grabPointInfo.SnapMode != UxrSnapToHandMode.DontSnap)
                {
                    foreach (var gripPoseInfo in grabPointInfo.GetCompatibleGripPoseInfos(avatar, usePrefabInheritance))
                    {
        ...
        {
            var grabPointIndices = new List<int>();

            for (var i = 0; i < GrabPointCount; ++i)
            {
                var grabPointInfo = GetGrabPoint(i);
                var gripPoseInfo = grabPointInfo.GetGripPoseInfo(avatar);
        ...
        {
            for (var grabPoint = 0; grabPoint < GrabPointCount; ++grabPoint)
            {
                for (var i = 0; i < GetGrabPoint(grabPoint).GripPoseInfoCount; ++i)
                {
                    var gripPoseInfo = GetGrabPoint(grabPoint).GetGripPoseInfo(i);
        ...
        public bool Editor_HasGrabPointWithGrabAlignTransform(Transform snapTransform, GameObject avatarPrefab)
        {
            for (var grabPoint = 0; grabPoint < GrabPointCount; ++grabPoint)
            {
                var gripPoseInfo = GetGrabPoint(grabPoint).GetGripPoseInfo(avatarPrefab.GetComponent<UxrAvatar>(), false);
