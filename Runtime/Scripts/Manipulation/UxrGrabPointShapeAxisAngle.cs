...
namespace UltimateXR.Manipulation
{
    ...
    public class UxrGrabPointShapeAxisAngle : UxrGrabPointShape
    {
        ...
        {
            var grabbableObject = GetComponent<UxrGrabbableObject>();
        ...
        public override void GetClosestSnap(UxrGrabber grabber, Transform snapTransform, Transform distanceTransform, Transform grabberDistanceTransform, out Vector3 position, out Quaternion rotation)
        {
            ...
            var worldAxis = Center.TransformDirection(_centerAxis);
            var localSnapAxis = snapTransform.InverseTransformDirection(worldAxis);
            var worldGrabberAxis = grabber.transform.TransformDirection(localSnapAxis);
            var reverseGrip = _bidirectional && Vector3.Angle(worldGrabberAxis, -worldAxis) < Vector3.Angle(worldGrabberAxis, worldAxis);
            ...
            var projection = Quaternion.FromToRotation(worldGrabberAxis, reverseGrip ? -worldAxis : worldAxis);
/*
            if (reverseGrip)
            {
            ...
            var worldPerpendicular = Center.TransformDirection(_centerAxis.Perpendicular);
            var localPerpendicular = snapTransform.InverseTransformDirection(worldPerpendicular);

            var grabberRotation = grabber.transform.rotation;
            grabber.transform.rotation = rotation;

            // Compute angle and clamp it.

            var angle = Vector3.SignedAngle(worldPerpendicular, grabber.transform.TransformDirection(localPerpendicular), worldAxis);
            var clampedAngle = Mathf.Clamp(angle, _angleMin, _angleMax);
            ...
            var projectedSnap = snapTransform.position.ProjectOnLine(Center.position, worldAxis);
            var fromAxisToSnap = snapTransform.position - projectedSnap;
